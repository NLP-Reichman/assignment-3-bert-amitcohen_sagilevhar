...
def test_prepare_data(results):
    if not tuple(results["dev_texts_shape"]) == (250, 61):
        return f"Dev texts shape is {results['dev_texts_shape']}, expected (250, 61)"
    if not tuple(results["train_labels_shape"]) == (1750, 77):
        return f"Train Labels shape is {results['train_labels_shape']}, expected (1750, 77)"
    return 1

def test_model_partial(results):
    f1 = results["f1"]
...
def test_model(results):
    ...
    if f1 < 0.97:
        return f"F1 is {f1}, expected at least 0.97"
    if f1_wo_o < 0.93:
        return f"F1 without O is {f1_wo_o}, expected at least 0.93"
    return 1
